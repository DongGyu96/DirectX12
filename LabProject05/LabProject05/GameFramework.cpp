...
bool CGameFramework::OnCreate(HINSTANCE hInstance, HWND hMainWnd)
{
	...
	//Direct3D 디바이스, 명령 큐와 명령 리스트, 스왑체인 등을 생성하는 함수를 호출한다.
...
void CGameFramework::CreateSwapChain()
{
	...
	dxgiSwapChainDesc.SampleDesc.Count = (m_bMsaa4xEndable) ? 4 : 1;
	dxgiSwapChainDesc.SampleDesc.Quality = (m_bMsaa4xEndable) ? (m_nMsaa4xQualityLevels - 1) : 0;
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	m_bMsaa4xEndable = (m_nMsaa4xQualityLevels > 1) ? true : false;
...
void CGameFramework::CreateCommandQueueAndList()
{
	...
	// 명령 리스트는 생성되면 열린 상태이므로 닫힌 상태로 만든다
}
...
void CGameFramework::CreateDepthStencilView()
{
	...
	d3dResourceDesc.SampleDesc.Count = (m_bMsaa4xEndable) ? 4 : 1;
	d3dResourceDesc.SampleDesc.Quality = (m_bMsaa4xEndable) ? (m_nMsaa4xQualityLevels - 1) : 0;
...
LRESULT CALLBACK CGameFramework::OnProcessingWindowMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	// WM_SIZE 메시지는 윈도우가 생성될 떄 한번 호출되거나 윈도우의 크기가 변경될 떄 호출된다. 주 윈도우의 크기
	// 를 사용자가 변경할 수 없으므로 윈도우의 크기가 변경되는 경우는 윈도우 모드와 전체화면 모드의 전환이 발생할 떄 이다.
	case WM_SIZE:
	{
...
{
	if (m_pScene) m_pScene->AnimationObjects(m_GameTimer.GetTimeElapsed());
}
...
void CGameFramework::FrameAdvance()
{
	...
	// 명령리스트를 명령 큐에 추가하여 실행한다

	WaitForGpuComplete();
	//GPU가 모든 명령 리스트를 실행할 떄 까지 기다린다
	...
	// 스왑체인을 프리젠트한다. 프리젠트를 하면 현재 렌더 타겟(후면 버퍼)의 내용이 전면 버퍼로 옮겨지고 
	// 렌더 타겟 인덱스가 바뀔 것이다.
	...
	// "LapProject ("으로 초기화되었으므로 (m_pszFrameRaate+12)에서부터 프레임 레이트를 문자열로 출력하여 " FPS)" 문자열과 합친다.
