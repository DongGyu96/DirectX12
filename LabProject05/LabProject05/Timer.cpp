...
// 와 성능 주파수(Performance Frequency)를 사용하여 현재 시간을 갱신하고 없으면 멀티미디어
// 타이머를 사용한다
void CGameTimer::Tick(float fLockFPS)
{
	...
	// 마지막 프레임 처리 시간과 현재 프레임 처리 시간의 차이가 1초보다 작으면 현재 프레임 처리 시간을
	...
	// 초당 프레임 수를 1 증가시키고 현재 프레임의 처리 시간을 누적하여 저장한다.
	m_nFramesPerSecond++;
...
unsigned long CGameTimer::GetFrameRate(LPTSTR IpszString, int nCharacters)
{
	// 현재 프레임 레이트를 문자열로 변환하여 IpszString 버퍼에 쓰고 "FPS"와 결합한다.
	if (IpszString)
	{
		_itow_s(m_nCurrentFrameRate, IpszString, nCharacters, 10);
		wcscat_s(IpszString, nCharacters, _T(" FPS)"));
	}
