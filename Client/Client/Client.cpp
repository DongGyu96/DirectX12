	...
	_In_ int       nCmdShow)
{
	...
	if (FAILED(pSystem->Add_Timer(L"Timer_60")))
		return FALSE;
	if (FAILED(pSystem->Add_Frame(L"Frame_60", 99999.f)))
		return FALSE;
	...
	while (1)
	{
		...
		if (pSystem->Permit_Call(L"Frame_60", fTimeDelta_Default))
		{
			_float fTime_Delta_60 = pSystem->Get_TimeDelta(L"Timer_60");

			pDevice->FrameAdvance(pMainApp, fTime_Delta_60);
