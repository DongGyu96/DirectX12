...
void CGameFramework::ProcessInput()
{
	...
	//	if (pKeyBuffer[VK_PRIOR] & 0xF0) dwDirection |= DIR_UP; // PgDn
	//	if (pKeyBuffer[VK_NEXT] & 0xF0) dwDirection |= DIR_DOWN; // PgUp
	//}
...
void CGameFramework::FrameAdvance()
{
	...
	XMFLOAT3 direction = Vector3::Subtract(m_pScene->m_ppObjects[m_pPlayer->railcount]->GetEnd(), m_pPlayer->GetPosition(), true);

	if (direction.y > 0.2f && m_pPlayer->movespeed > 15.f)
		m_pPlayer->movespeed -= 0.25f;
	else if (direction.y < -0.2f && m_pPlayer->movespeed < 50.f)
		m_pPlayer->movespeed += 0.25f;

	/*XMFLOAT4X4 tempMatrix;

	tempMatrix = Matrix4x4::RotationYawPitchRoll(-m_pPlayer->xangle, m_pPlayer->yangle, 0.0f);

	XMFLOAT3 direction1 = Vector3::Normalize(Vector3::TransformNormal(XMFLOAT3(0.0f, 0.0f, 1.0f), tempMatrix));*/

	XMFLOAT3 temp = Vector3::Add(m_pPlayer->GetPosition(), Vector3::ScalarProduct(direction, (m_pPlayer->movespeed * m_GameTimer.GetTimeElapsed())));
	m_pPlayer->SetPosition(temp.x, temp.y, temp.z);
