...
// 입력 조립기에서 정점 버퍼의 구조를 알려주기 위한 구조체를 반환한다.
D3D12_INPUT_LAYOUT_DESC CShader::CreateInputLayout()
{
	...
	pd3dInputElementDescs[0] = { "COLOR", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 12, 
		D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
...
void CShader::AnimateObjects(float fTimeElapsed)
{
	...
	{
		m_ppObjects[j]->AnimateObjects(fTimeElapsed);
...
void CShader::ReleaseUploadBuffers()
{
	if (m_ppObjects)
	{
		for (int j = 0; j < m_nObjects; j++) if (m_ppObjects[j]) m_ppObjects[j]->ReleaseUploadBuffer();
	}
}
